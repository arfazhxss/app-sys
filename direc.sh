...
list_structure() {
  ...
  # List the files in the current directory, excluding dotfiles
  find "$dir" -maxdepth 1 -type f ! -name '.*' | while IFS= read -r file; do
    echo "${indent}├── $(basename "$file")" >> "$output_file"
  done

  # List the subdirectories in the current directory, excluding dotfiles and node_modules
  find "$dir" -maxdepth 1 -type d ! -path "$dir" ! -name '.*' ! -name 'node_modules' | while IFS= read -r subdir; do
